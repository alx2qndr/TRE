...
Swapchain::~Swapchain()
{
    ...
    for (int i = 0; i < depthImages.size(); i++) {
        vkDestroyImageView(device.logicalDevice, depthImageViews[i], nullptr);
        vkDestroyImage(device.logicalDevice, depthImages[i], nullptr);
        vkFreeMemory(device.logicalDevice, depthImageMemorys[i], nullptr);
    }
...
void Swapchain::createImageViews()
{
    ...
    for (size_t i = 0; i < swapchainImages.size(); i++) {
        ...
        if (vkCreateImageView(device.logicalDevice, &viewInfo, nullptr, &swapchainImageViews[i]) != VK_SUCCESS) {
...
void Swapchain::createDepthResources()
{
    ...
    for (int i = 0; i < depthImages.size(); i++) {
        ...
        if (vkCreateImageView(device.logicalDevice, &viewInfo, nullptr, &depthImageViews[i]) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create texture image view!");
        }
    }
}
...
void Swapchain::createSyncObjects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(device.logicalDevice, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS || vkCreateSemaphore(device.logicalDevice, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS || vkCreateFence(device.logicalDevice, &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS) {
