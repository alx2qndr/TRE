...
namespace tre {
Renderer::Renderer(Context& contextRef, Device& deviceRef, Window& windowRef)
    : context(contextRef)
    , device(deviceRef)
    , window(windowRef)
{
...
void Renderer::recreateSwapchain()
{
    ...
    if (swapchain == nullptr) {
        swapchain = std::make_unique<Swapchain>(context, device, extent);
    } else {
        std::shared_ptr<Swapchain> oldSwapChain = std::move(swapchain);
        swapchain = std::make_unique<Swapchain>(context, device, extent, oldSwapChain);
...
VkCommandBuffer Renderer::beginFrame()
{
    ...
    auto result = swapchain->acquireNextImage(&currentFrameIndex);
...
void Renderer::endFrame()
{
    ...
    auto result = swapchain->submitCommandBuffers(&commandBuffer, &currentFrameIndex);
...
{
    assert(frameStarted && "Can't call beginSwapChainRenderPass if frame is not in progress");
    assert(
        commandBuffer == getCurrentCommandBuffer() && "Can't begin render pass on command buffer from a different frame");
